...
async def test_firecrawl_extract_agent():
    ...
    mock_page = BrowsedPage(
        ...
        content="This is a mock page content with more than fifty words to satisfy the length filters. " * 5,
    ...
    with patch("services.firecrawl_service.FirecrawlService.batch_scrape", new_callable=AsyncMock) as mock_batch:
        mock_batch.return_value = [mock_page]
        
        input_data = {
            "results": [{"url": "https://example.com/test", "relevance_score": 0.9, "source_quality": 0.8}],
            "max_pages": 5,
        }
