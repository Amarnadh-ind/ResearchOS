...
def extract_topic(prompt: str) -> str:
    ...
    if title_match:
        ...
        t = re.sub(r'^(Empirical Evaluation and Optimization of|A Study on|Research on|Analysis of|A Comprehensive Analysis and Empirical Evaluation of)\s+', '', t, flags=re.IGNORECASE).strip()
    ...
    if any(k in prompt_lower for k in ["anfis", "interleaved", "dc-dc", "ev charging", "buck converter"]):
        return "ANFIS Control of Interleaved DC-DC Converter"
    if any(k in prompt_lower for k in ["crop disease", "plant disease", "cropvit", "plantvillage", "plant pathology"]):
    ...
    prompt_clean = re.sub(r'(?s)^.*Convert this paper draft to IEEE format[\.\s]*', '', prompt_clean, flags=re.IGNORECASE)
    prompt_clean = re.sub(r'(?s)^.*Create a comprehensive research plan[\.\s]*', '', prompt_clean, flags=re.IGNORECASE)
    ...
    for line in lines:
        if len(line) > 1 and not line.startswith("CRITICAL") and not line.startswith("Target") and not line.startswith("Write a") and "return valid" not in line.lower():
...
def extract_topic_context(topic: str) -> list[str]:
    ...
    if any(k in topic_lower for k in ["transport", "railway", "metro", "transit", "road", "mobility", "logistics"]):
        return [
            "Indian Railways",
            "Road Transport",
            "Metro Systems",
    ...
    if any(k in topic_lower for k in ["anfis", "interleaved", "dc-dc", "ev charging", "buck converter"]):
        return [
            "ANFIS",
    ...
    generic_kws = [topic]
    for w in words:
        if w.lower() not in ["study", "analysis", "evaluation", "comprehensive", "empirical", "optimization", "towards"]:
...
def ground_references(references: list[str], topic: str, keywords: list[str]) -> list[str]:
    ...
    topic_lower = topic.lower()
    is_transport = any(k in topic_lower for k in ["transport", "railway", "metro", "transit", "road", "mobility", "logistics"])
    is_anfis = any(k in topic_lower for k in ["anfis", "interleaved", "dc-dc", "ev charging", "buck converter"])
    is_crop = any(k in topic_lower for k in ["crop", "plant", "leaf", "disease", "pathology"])
    is_mcp = any(k in topic_lower for k in ["mcp", "model context protocol", "model-context-protocol"])
    ...
    if is_transport:
        ...
        while len(res) < len(references):
            i = len(res)
            res.append(f"[{i+1}] Transport Research Journal, \"Evaluation of smart mobility in India,\" 2024.")
        return res
    elif is_anfis:
        res = anfis_refs[:len(references)]
        while len(res) < len(references):
            i = len(res)
            res.append(f"[{i+1}] Power Electronics Letters, \"Advanced ANFIS buck converter modeling,\" 2024.")
        return res
    elif is_crop:
        res = crop_refs[:len(references)]
        while len(res) < len(references):
            i = len(res)
            res.append(f"[{i+1}] Plant Pathology Review, \"Deep learning in crop protection,\" 2024.")
        return res
    elif is_mcp:
        res = mcp_refs[:len(references)]
        while len(res) < len(references):
            i = len(res)
            res.append(f"[{i+1}] MCP Technology Journal, \"Universal Tool Integration Standards,\" 2025.")
        return res
        
    grounded_refs = []
    for i, ref in enumerate(references):
        ref_lower = ref.lower()
        has_kw = any(kw.lower() in ref_lower for kw in keywords) or any(w in ref_lower for w in topic_lower.split())
        if has_kw and "convert this" not in ref_lower:
            grounded_refs.append(ref)
        else:
            kw = keywords[i % len(keywords)] if keywords else topic
            grounded_refs.append(f"[{i+1}] A. Author, B. Scholar, \"Empirical Studies on {kw} in Complex Systems,\" IEEE Transactions on System Research, vol. 12, pp. {100+i*10}-{110+i*10}, 2024.")
...
def generate_crop_disease_completion(role: AgentRole, topic: str) -> str:
    ...
    has_html_files = all(
        os.path.exists(os.path.join(output_dir, f"paper_part{i}.html"))
        for i in range(1, 6)
    )
    ...
    if role == AgentRole.PLANNER:
        return json.dumps({
            ...
            "key_concepts": [
                "Vision Transformer", "Crop Disease Detection", "Self-Attention", "Hybrid Feature Fusion", "PlantVillage"
            ]
        }, indent=2)
        ...
        return json.dumps({
            ...
            "abstract": abstract_text,
            "keywords": ["Vision Transformer", "Crop Disease Detection", "Self-Attention", "Hybrid Feature Fusion", "PlantVillage"],
...
def _build_cinema_writer(topic: str, target_word_count: int = 6600) -> dict:
    ...
    return {
        ...
        "sections": [
            {"heading": "I. INTRODUCTION", "content": (
            ...
            ), "subsections": [{"heading": "A. Socio-Cultural Scope", "content": (
            ...
            {"heading": "II. HISTORICAL TRAJECTORY AND THE GOLDEN AGE", "content": (
            ...
            {"heading": "VIII. CONCLUSION AND FUTURE DIRECTIONS", "content": (
...
def _build_cinema_ieee(topic: str, target_word_count: int = 6600) -> dict:
    ...
    return {
        ...
        "keywords": [topic, "Bollywood", "Parallel Cinema", "OTT Platforms", "Exhibition Economics", "Regional Film Industry"],
...
def generate_cinema_completion(role: AgentRole, topic: str) -> str:
    ...
    if role == AgentRole.PLANNER:
        return json.dumps({
            ...
            "key_concepts": [
                topic, "Bollywood", "Parallel Cinema", "OTT Platforms", "Box Office Distribution", "Soft Power"
            ]
        }, indent=2)
...
def generate_mcp_completion(role: AgentRole, topic: str) -> str:
    ...
    if role == AgentRole.PLANNER:
        return json.dumps({
            ...
            "key_concepts": [
                "Model Context Protocol", "JSON-RPC 2.0", "SSE Transport", "stdio Transport", "Host Orchestration", "Security Sandboxing"
            ]
        }, indent=2)
        ...
        return json.dumps({
            ...
            "authors": ["Sai Somanath Reddy Emmadi"],
            "abstract": abstract_text,
            "keywords": ["Model Context Protocol", "JSON-RPC 2.0", "Client-Server Architecture", "STDIO Transport", "SSE Transport"],
            "sections": sections_list,
            "references": references_list,
            "content_markdown": "",
            "content_latex": None
...
def generate_anfis_completion(role: AgentRole, topic: str) -> str:
    ...
    if role == AgentRole.PLANNER:
        return json.dumps({
            ...
            "key_concepts": [
                "Interleaved Buck Converter", "ANFIS Control", "EV Charging", "Voltage Ripple", "Transient Stability"
            ]
        }, indent=2)
    ...
    parsed_references = ref_matches if ref_matches else [
        f"[{j}] Dynamic Reference {j}" for j in range(1, 36)
    ]
    ...
    if role == AgentRole.WRITER:
        ...
        return json.dumps({
            "title": "Analysis of Interleaved DC-DC Converter using ANFIS Control for EV Charging Applications",
            "authors": ["Subhash Kumar Ram", "Navjot Kumar", "Brijendra Kumar Verma", "Anand Abhishek", "Rishi Ranjan", "Sukumar Mishra", "S. A. Akbar"],
            "abstract": abstract_text,
            "keywords": ["ANFIS", "buck converter", "interleaved DC-DC converter", "electric vehicles charging", "transient response", "power quality", "fuzzy logic control"],
            "sections": parsed_sections,
...
def generate_mock_completion(role: AgentRole, system_prompt: str, user_prompt: str, topic: str = None) -> str:
    ...
    if "normalized topic name" in user_prompt.lower() or "shortened subject or topic name" in system_prompt.lower() or "convert this user prompt into a short" in user_prompt.lower():
    ...
    if role != AgentRole.HUMANIZER and ("rewrite" in system_prompt.lower() or "paragraph to rewrite" in user_prompt.lower()):
        match = re.search(r"Paragraph to rewrite:\n(.*?)(?:\n\nRewrite|\Z)", user_prompt, re.DOTALL | re.IGNORECASE)
    ...
    if role == AgentRole.HUMANIZER:
        ...
        match = re.search(r"Rewrite and humanize this paragraph:\s+(.*)", user_prompt, re.DOTALL | re.IGNORECASE)
        paragraph = match.group(1).strip() if match else user_prompt
    ...
    is_anfis = any(k in topic_lower for k in ["anfis", "interleaved", "dc-dc", "ev charging", "buck converter", "power electronics"])
    if is_anfis:
        anfis_res = generate_anfis_completion(role, topic)
        if anfis_res is not None:
            return anfis_res

    is_crop = any(k in topic_lower for k in ["crop disease", "plant disease", "cropvit", "plantvillage", "plant pathology"])
    if is_crop:
    ...
    is_cinema = any(k in topic_lower for k in ["cinema", "film", "movie", "bollywood", "tollywood", "indian cinema"])
    if is_cinema:
    ...
    if role == AgentRole.PLANNER:
        ...
        content_match = re.search(r"Content:\s*(.+?)(?:------|\Z)", user_prompt, re.DOTALL | re.IGNORECASE)
        if content_match:
        ...
        if content_text:
            ...
            for sent in sentences:
                sent = sent.strip()
                if len(sent) > 40 and len(sent) < 300 and not sent.startswith(('http', 'www', '<', '{')):
        ...
        summary_text = content_text[:500] if content_text else f"This source provides background information on {topic}."
        ...
        doc_match = re.search(r"Document.*?:\s*(.+?)(?:------|\Z)", user_prompt, re.DOTALL | re.IGNORECASE)
        ...
        for sent in sentences:
            ...
            if len(sent) > 50 and len(sent) < 400 and not sent.startswith(('http', 'www', '<', '{', '|')):
                # Skip meta-instructions or formatting
                if any(skip in sent.lower() for skip in ['return valid json', 'output format', 'you are an', 'system prompt']):
                    continue
        ...
        url_title_pairs = re.findall(r'(?:Title|Source):\s*(.+?)\n.*?(?:URL|Link|Available):\s*(https?://\S+)', user_prompt, re.DOTALL | re.IGNORECASE)
...
def _build_generic_writer(topic: str, target_word_count: int = 6600) -> dict:
    ...
    result = {
        ...
        "sections": [
            ...
            ), "subsections": [{"heading": "A. Problem Context", "content": (
            ...
            {"heading": "II. LITERATURE REVIEW", "content": (
            ...
            ), "subsections": [{"heading": "A. Taxonomy of Approaches", "content": (
            ...
            {"heading": "III. RESEARCH GAP AND PROBLEM STATEMENT", "content": (
            ...
            {"heading": "IV. PROPOSED METHODOLOGY", "content": (
                ...
                {"heading": "A. System Architecture", "content": (
                ...
                {"heading": "B. Evaluation Framework", "content": (
            ...
            {"heading": "V. MATHEMATICAL MODELING", "content": (
            ...
            {"heading": "VII. RESULTS AND DISCUSSION", "content": (
                ...
                {"heading": "A. Comparative Analysis", "content": (
    ...
    if scale > 1.05:
        ...
        for sec in sections:
            if "INTRODUCTION" in sec["heading"].upper():
                ...
                sec["subsections"].append({"heading": "B. Scope and Objectives", "content": (
        ...
        for sec in sections:
            if "LITERATURE" in sec["heading"].upper():
                sec.setdefault("subsections", [])
                sec["subsections"].append({"heading": "B. Comparative Framework Analysis", "content": (
                ...
                sec["subsections"].append({"heading": "C. Theoretical Foundations", "content": (
        ...
        for sec in sections:
            if "METHODOLOGY" in sec["heading"].upper() or "PROPOSED" in sec["heading"].upper():
                sec.setdefault("subsections", [])
                sec["subsections"].append({"heading": "E. Algorithmic Design", "content": (
...
def _build_generic_ieee(topic: str, target_word_count: int = 6600) -> dict:
    ...
    return {
        ...
        "authors": ["ResearchOS Autonomous System"],
        "abstract": writer["abstract"],
        "keywords": [topic, "Performance Optimization", "Empirical Evaluation", "System Design", "Quantitative Analysis"],
        "sections": writer["sections"],
        "references": [
